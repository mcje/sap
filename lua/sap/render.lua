    ...
    vim.api.nvim_set_decoration_provider(ns, {
        on_line = function(_, _, bufnr, row)
            ...
            if guides_cfg and guides_cfg.enabled and ftype == "directory" then
                ...
                local indicator = is_expanded and guides_cfg.icons.expanded or guides_cfg.icons.collapsed
...
--- Surgical collapse: remove children lines, store in hidden_content
---@param bufnr integer
---@param state State
---@param entry Entry
---@return boolean success
function M.collapse(bufnr, state, entry)
    local entry_line = M.find_line_by_id(bufnr, entry.id)
    ...
    for _, p in ipairs(parsed) do
        ...
        if p.path:match("^" .. vim.pesc(entry.path) .. "/") then
            hidden_entries[#hidden_entries + 1] = p
        end
    end
    ...
    local start_line = hidden_entries[1].linenr
    local end_line = hidden_entries[#hidden_entries].linenr

    -- Get raw lines for restoration
    local raw_lines = vim.api.nvim_buf_get_lines(bufnr, start_line - 1, end_line, false)

    -- Build HiddenEntry list with both parsed data and raw lines
    local to_store = {}
    for i, p in ipairs(hidden_entries) do
        to_store[#to_store + 1] = {
            id = p.id,
            name = p.name,
            type = p.type,
    ...
    state:save_hidden_content(entry.path, to_store)
...
--- Surgical expand: restore from hidden_content or load fresh, insert lines
---@param bufnr integer
---@param state State
---@param entry Entry
---@return boolean success
...
function M.expand(bufnr, state, entry)
    ...
    local hidden = state:get_hidden_content(entry.path)
    local new_lines

    if hidden and #hidden > 0 then
        -- Restore from hidden content (preserves user edits)
        new_lines = {}
        for _, h in ipairs(hidden) do
            new_lines[#new_lines + 1] = h.line
        end
        state:clear_hidden_content(entry.path)
    else
...
function M.go_to_parent(bufnr, state)
    ...
    -- Shift all existing lines deeper by one indent level
    local indent_size = get_indent_size()
    shift_lines_indent(bufnr, 1, old_line_count, indent_size)

    -- Get new root entry and its children (siblings of old root)
    ...
    local old_root_name = vim.fs.basename(old_root_path)
    for _, sibling in ipairs(siblings) do
        if sibling.path == old_root_path then
            -- This is the old root, already in buffer (shifted), skip
            break
        end
    ...
    -- It starts at old_root_will_be_at and includes all deeper-indented lines
...
function M.set_root(bufnr, state, entry)
    ...
    -- Parse buffer to get paths for hidden content
    local parsed = parser.parse_buffer(bufnr, state.root_path)

    -- Store lines that will become hidden (above new root AND below subtree)
    local hidden_entries = {}
    for _, p in ipairs(parsed) do
        if p.linenr < entry_line or p.linenr > subtree_end then
            hidden_entries[#hidden_entries + 1] = {
                id = p.id,
                name = p.name,
                type = p.type,
                path = p.path,
    ...
    -- Store under old root
    if #hidden_entries > 0 then
        state:save_hidden_content(state.root_path, hidden_entries)
    end
